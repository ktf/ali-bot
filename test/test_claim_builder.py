...
class ClaimBuilderFastFailTestCase(unittest.TestCase):
    ...
    def test_the_failing_check_stops_being_attempted(self):
        """Three fast failures and that check is left alone for a while."""
        self.assertLessEqual(len(self._bad()), 3,
                             "expected the failing check to cool down after 3 "
                             "fast failures; it was attempted %d times: %r"
                             % (len(self._bad()), self.ORDER))

    def test_the_worker_keeps_building_the_healthy_check(self):
        """The whole point: a cooled check must not idle the worker.

        If this ever fails while the test above passes, the cooldown has been
        reattached to the worker instead of the check, and a fleet-wide
        breakage will stop every builder at once.
        """
        self.assertGreater(len(self._good()), len(self._bad()),
                           "the healthy check should have kept building; got %r"
                           % (self.ORDER,))
