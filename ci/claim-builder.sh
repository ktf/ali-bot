...
start_heartbeat() {          # round check pr sha
  ...
  ( set +x
    ...
    while sleep "$HEARTBEAT"; do
      # Timestamped in aliBuild's own format. stdout and stderr are captured
      # into SEPARATE files, so their relative order is lost -- a heartbeat and
      # the compiler output it describes cannot be lined up by position, only
      # by clock. Matching the format the build already prints
      # (2026-08-20@07:15:03) means one `sort` merges the two streams.
    ...
    while read -r build_type pr_number pr_hash env_name waiting_since; do
      ...
      if [ -e "$marker" ]; then
        ...
        printf '%s ===== ROUND %s END pr=%s result=claim-lost =====\n' \
               "$(date -u '+%Y-%m-%d@%H:%M:%S')" "$round" "$pr_number"
      fi
