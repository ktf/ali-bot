...
#   FAST_FAIL_MAX_COOLING  CEILING: if more than this many checks are already
#                       cooling, cooldowns stop being honoured and the worker
#                       takes work anyway (2). Without it a worker facing a
#                       fleet-wide breakage cools every check in turn and goes
#                       idle -- which is the starvation this design exists to
#                       avoid, arrived at the long way round.
...
EOF
      ...
      while IFS='|' read -r _f_check _f_n _f_when; do
        ...
        _cooling=$((_cooling + 1))
        [ "$_f_check" = "$check" ] && _skip=1
      done <<EOF
$fastfail
EOF
      # The ceiling. Past it the worker stops honouring cooldowns entirely:
      # better to keep taking work that may fail than to sit idle while the
      # queue grows.
      if [ "$_cooling" -gt "$FAST_FAIL_MAX_COOLING" ]; then
        _skip=
      fi
      unset _f_check _f_n _f_when _now _cooling
      if [ -n "$_skip" ]; then
        unset _skip
        continue
      fi
