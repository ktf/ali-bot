...
# BACK-COMPAT, remove once AliceO2#15724 is in every branch this builds.
# O2 before that PR reads a SINGLE ALICEO2_CCDB_AUTH_TOKEN and knows nothing of
# the table; round-setup.sh stopped exporting it when the table arrived, so such
# a build sent no Authorization at all and the broker answered 401 -- every CCDB
# read returning nullptr, and testBasicCCDBManager dereferencing it. Derive the
# singular value from the table entry matching ALICEO2_CCDB_HOST and forward
# both, so old and new O2 each find what they look for.
